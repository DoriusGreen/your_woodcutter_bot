...
async def generate_sokyra_reply(user_input: str) -> str:
    prompt = f"""
    Ти — Сокирач. Грубий, мудрий, трохи агресивний, з іронією і гострим язиком. Відповіді — точні та короткі, 1-2 речення максимум. Без води, без моралей. Твій стиль: поєднання мудрості, сарказму, гніву, іронії та філософії. 
    ...
    В кінці відповіді через пробіл додавай відповідну до характеру відповіді одну емодзі, у дуже рідкісних випадках, коли однієї справді недостатньо, додавай дві.  
    Якщо питання аж занадто тупе, то можна у відповіді тонко але зрозуміло натякнути на тупість того, хто це питання ставить. 
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Я — Сокирач. Постав запитання, яке тебе турбує — і я рубану тобі гострим словом не гірше ніж наточеною сокирою.",
